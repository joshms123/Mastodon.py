...
class Mastodon:
    ...
    def status_reply(self, to_status, status, in_reply_to_id=None, media_ids=None,
        ...
        status = (
            "".join(map(lambda x: f"@{x} ", mentioned_accounts.values())) + status
        )
    ...
    def stream_healthy(self):
        ...
        return api_okay in [b'OK', b'success']
        ...
        for k, v in json_object.items():
            if k in known_date_fields:
                if v != None:
                    try:
                        json_object[k] = (
                            datetime.datetime.fromtimestamp(v, pytz.utc)
                            if isinstance(v, int)
                            else dateutil.parser.parse(v)
                        )
                    except:
